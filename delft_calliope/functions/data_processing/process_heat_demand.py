"""
Heat Demand Processing Module

This module fetches residential heat demand data, merges it with building data,
...
from functions.data_acquisition import fetch_residential_heat_demand
...
def process_heat_demand(buildings_df, area, year, mode='plot', online=True, csv_path='inputs/heat_demand_cache', debug_folder='debug'):
    ...
    buildings_df : pd.DataFrame
        DataFrame containing building information with geometry and coordinates.
    area : str
        Area code for fetching heat demand data (e.g., '4341' for Mythologiebuurt 2020).
    year : int
        Year for fetching heat demand data (used only for online API calls).
    mode : str
        If 'plot', creates an interactive heat demand map visualization. Default is 'plot'.
    online : bool
    ...
    debug_folder : str
        Folder for debug visualizations (default: 'debug').
    
    Returns
    -------
    tuple
        (merged_df, buildings_gdf)
