"""
Stedin Grid Processing Module

This module fetches and processes Stedin heat and electricity grid data from
ArcGIS REST API or cache files, with support for both Stedin and OSM topology sources.
"""
...
from functions.network_topology.process_osm_roads import extract_osm_roads
...
def fetch_stedin_layer_from_arcgis(bbox_coords, layer_url, layer_name="Layer", max_records=2000):
    ...
    bbox_coords : list of tuples
        Polygon coordinates [(lon, lat), ...] in WGS84
    layer_url : str
        ArcGIS Feature Service layer URL 
    layer_name : str
        Descriptive name for logging
    max_records : int
        Maximum records per request (ArcGIS limit is usually 2000)
    
    Returns
    -------
    GeoDataFrame
        Spatial features from the service in EPSG:4326
    """
    ...
    try:
        ...
        logger.warning(
            "Stedin API request failed for %s: %s. Returning %d partial features.",
            layer_name, e, len(all_features)
        )
        if len(all_features) > 0:
...
def load_stedin_grids_from_cache(bbox_coords, cache_path='inputs/stedin_cache'):
    ...
    -------
    tuple
        (stedin_gas_gdf, stedin_elec_gdf, stedin_transformers_gdf) - filtered to bounding box
...
def visualize_network_topology(heat_gdf, elec_gdf, transformers_gdf, bbox_coords, 
    ...
    Parameters
    ----------
    heat_gdf : GeoDataFrame
        Heat/gas network features
    elec_gdf : GeoDataFrame
        Electricity network features
    transformers_gdf : GeoDataFrame
        Transformer locations
    bbox_coords : list of tuples
        Bounding box coordinates for fallback centering
    heat_label : str
        Label for heat network layer (default: "Heat Network")
    elec_label : str
        Label for electricity network layer (default: "Electricity Network")
    output_file : str
        Path to save the HTML map (default: "debug/topology_map.html")
    debug_folder : str
        Folder for debug output (default: 'debug')
    """
...
def filter_features_within_buildings(gdf, buildings_gdf, buffer_distance=0.0001):
    ...
    ----------
    gdf : GeoDataFrame
        Grid features (lines) in EPSG:4326
    buildings_gdf : GeoDataFrame
        Building polygons in EPSG:28992
    buffer_distance : float
        Small buffer in degrees to avoid edge cases (default: ~10m)
    
    Returns
    -------
    GeoDataFrame
        Features with those inside buildings removed
    """
...
def process_stedin_grids(
    ...
    ----------
    bbox_coords : list of tuples
        Polygon coordinates defining the area of interest
    buildings_df : pd.DataFrame, optional
        Buildings dataframe to filter features within buildings
    features_to_remove_heat : list of str
        Heat/gas network feature names to exclude
    features_to_remove_elec : list of str
        Electricity network feature names to exclude
    mode : str
        If 'plot', creates interactive map visualizations. Default is 'plot'
    online : bool
        If True, fetch from Stedin API; if False, load from cache. Default is True
    cache_path : str
        Path to cache directory (used when online=False). Default is 'inputs/stedin_cache'
    base_service_url : str
        Base URL for Stedin ArcGIS FeatureServer
    gas_layer_id : int
        Layer ID for gas network (default: 1)
    lv_elec_layer_id : int
        Layer ID for low voltage electricity grid (default: 2)
    transformer_layer_id : int
        Layer ID for MV-LV transformers (default: 6)
    simplify_tolerance : float
        Geometry simplification precision (in degrees)
    snap_tolerance : float
        Geometry snapping threshold (in degrees)
    
    Returns
    -------
    tuple
        (stedin_heat_gdf_delft, stedin_elec_gdf_delft, stedin_transformers_gdf_delft)
    ...
    if not stedin_heat_gdf.empty:
        ...
        # Remove features within buildings
...
def process_network_topology(
    ...
    buildings_df : pd.DataFrame, optional
        Buildings dataframe to filter features within buildings
    topology_source : str
        'stedin' or 'osm' - source for network topology
    osm_pbf_path : str
        Path to OSM PBF file (used when topology_source='osm')
    features_to_remove_heat : list
        Heat/gas network feature names to exclude (Stedin only)
    features_to_remove_elec : list
        Electricity network feature names to exclude (Stedin only)
    mode : str
        If 'plot', creates interactive map visualizations
    online : bool
        If True, fetch from Stedin API; if False, load from cache
    cache_path : str
        Path to cache directory (used when online=False)
    base_service_url : str
        Base URL for Stedin ArcGIS FeatureServer
    gas_layer_id : int
        Layer ID for gas network (default: 1)
    ...
    -------
    tuple
        (heat_gdf, elec_gdf, transformers_gdf)
