...
def create_transmission_nodes(stedin_heat_gdf_delft, stedin_elec_gdf_delft, spacing_m=None):
    ...
    Args:
        stedin_heat_gdf_delft (GeoDataFrame): Cleaned heat/gas network with LineString geometries.
        stedin_elec_gdf_delft (GeoDataFrame): Cleaned electricity network with LineString geometries.
        spacing_m (float or None): Spacing in meters for node interpolation along line segments.
            - If provided (e.g., 3.5): Interpolates nodes at this spacing
            - If None: Returns only corner/endpoint nodes (no interpolation)
    
    Returns:
        tuple: (heat_interp_gdf, elec_interp_gdf)
